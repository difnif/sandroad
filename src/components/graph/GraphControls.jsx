...
import { Target, Palette, Eye, RotateCcw, Box, Circle } from 'lucide-react';
import { useTheme } from '../../contexts/ThemeContext.jsx';

const TILT_PRESETS = [0, 5, 10, 15, 20, 30];

export default function GraphControls({ mode, onModeChange, tilt, onTiltChange, viewMode, onViewModeChange }) {
  const { theme, themeId } = useTheme();
  const monoCls = theme.fontMono ? 'font-mono-ui' : '';
  const isVenn = viewMode === 'venn';
    ...
    <div className="flex flex-wrap items-center gap-2">
      ...
      <div className="flex items-center gap-0.5">
        <button
          ...
          className={`px-2 py-1 rounded border text-[11px] flex items-center gap-1 ${monoCls} ${
            !isVenn ? theme.buttonPrimary : theme.button
          }`}
          title="3D box view"
        >
          <Box size={11} /> 3D
        ...
        <button
          onClick={() => onViewModeChange('venn')}
          className={`px-2 py-1 rounded border text-[11px] flex items-center gap-1 ${monoCls} ${
            isVenn ? theme.buttonPrimary : theme.button
          }`}
          title={themeId === 'sand' ? '벤 다이어그램' : 'Venn diagram'}
        >
          <Circle size={11} /> venn
        </button>
      </div>
