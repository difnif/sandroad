...
import {
  computeClusterLayout, getColorHex, getRegionColor,
...
const REPARENT_DISTANCE = 5; // how close to drop on a box to reparent
...
export default function GraphScene({
  ...
  // Update hierarchy lines to follow box positions (live during drag)
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      let prevX = 0, prevY = 0, pointerDownPos = null, pointerDownTime = 0, lastTapTime = 0;
      let lastTouchDist = 0, lastTouchCenter = null;

      const getMouseVec = (cx, cy) => {
        const r = renderer.domElement.getBoundingClientRect();
        mv.x = ((cx - r.left) / r.width) * 2 - 1;
        mv.y = -((cy - r.top) / r.height) * 2 + 1;
        return mv;
      };
      const hitBoxes = (cx, cy) => {
        getMouseVec(cx, cy); raycaster.setFromCamera(mv, camera);
        const ints = raycaster.intersectObjects(Object.values(boxesRef.current));
        return ints.length > 0 ? ints[0] : null;
      };
      const projectToPlane = (cx, cy) => {
        getMouseVec(cx, cy); raycaster.setFromCamera(mv, camera);
        const t = new THREE.Vector3();
        raycaster.ray.intersectPlane(dragPlaneRef.current, t);
        return t;
      };

      // Find closest box to a world position (for reparent)
      const findClosestBox = (worldX, worldY, excludeId) => {
        let closest = null, minDist = REPARENT_DISTANCE;
        Object.entries(boxesRef.current).forEach(([id, mesh]) => {
          if (id === excludeId) return;
          const dx = mesh.position.x - worldX, dy = mesh.position.y - worldY;
      ...
      const onPointerDown = (e) => {
        ...
        pointerDownPos = { x: e.clientX, y: e.clientY }; pointerDownTime = Date.now();
        prevX = e.clientX; prevY = e.clientY;
        const hit = hitBoxes(e.clientX, e.clientY);
        if (hit) {
          interactionMode = 'dragBox'; dragBoxId = hit.object.userData.id;
          const p = projectToPlane(e.clientX, e.clientY);
          dragOffset.x = hit.object.position.x - p.x; dragOffset.y = hit.object.position.y - p.y;
        } else { interactionMode = 'rotate'; }
      };
      const onPointerMove = (e) => {
        ...
        if (interactionMode === 'dragBox' && dragBoxId) {
          ...
          if (mesh) { mesh.position.x = p.x + dragOffset.x; mesh.position.y = p.y + dragOffset.y; updateHierLines(); }
        } else if (interactionMode === 'rotate') {
          tiltRef.current += (e.clientX - prevX) * 0.3;
          pitchRef.current = Math.max(-60, Math.min(60, pitchRef.current + (e.clientY - prevY) * 0.3));
          prevX = e.clientX; prevY = e.clientY;
          updateCamera(); cbRef.current.onTiltChange?.(Math.round(tiltRef.current));
        }
      };
      const onPointerUp = (e) => {
        if (e.pointerType === 'touch') return;
        const moved = pointerDownPos ? Math.hypot(e.clientX - pointerDownPos.x, e.clientY - pointerDownPos.y) : 999;
        if (interactionMode === 'dragBox' && dragBoxId) {
          if (moved < 5) {
            const now = Date.now();
            if (now - lastTapTime < 400) cbRef.current.onRequestInlineEdit?.(dragBoxId);
            else cbRef.current.onSelectNode?.(dragBoxId);
            lastTapTime = now;
          } else {
            const mesh = boxesRef.current[dragBoxId];
            if (mesh) {
              // Check reparent
              const closest = findClosestBox(mesh.position.x, mesh.position.y, dragBoxId);
              if (closest) cbRef.current.onReparent?.(dragBoxId, closest.id);
              cbRef.current.onPositionChange?.(dragBoxId, { x: mesh.position.x, y: mesh.position.y });
            }
          }
        } else if (interactionMode === 'rotate' && moved < 5) {
          cbRef.current.onSelectNode?.(null);
        }
        interactionMode = null; dragBoxId = null; pointerDownPos = null;
      };

      // Touch
      const onTouchStart = (e) => {
        if (e.touches.length === 1) {
          const t = e.touches[0]; pointerDownPos = { x: t.clientX, y: t.clientY }; pointerDownTime = Date.now();
          prevX = t.clientX; prevY = t.clientY;
          const hit = hitBoxes(t.clientX, t.clientY);
          if (hit) {
            interactionMode = 'dragBox'; dragBoxId = hit.object.userData.id;
            const p = projectToPlane(t.clientX, t.clientY);
            dragOffset.x = hit.object.position.x - p.x; dragOffset.y = hit.object.position.y - p.y;
          } else { interactionMode = 'rotate'; }
        } else if (e.touches.length === 2) {
          interactionMode = 'pinch'; dragBoxId = null;
      ...
      const onTouchMove = (e) => {
        ...
        if (e.touches.length === 1) {
          ...
          if (interactionMode === 'dragBox' && dragBoxId) {
            ...
            tiltRef.current += (t.clientX - prevX) * 0.3;
            pitchRef.current = Math.max(-60, Math.min(60, pitchRef.current + (t.clientY - prevY) * 0.3));
            prevX = t.clientX; prevY = t.clientY;
            updateCamera(); cbRef.current.onTiltChange?.(Math.round(tiltRef.current));
          }
        } else if (e.touches.length === 2 && interactionMode === 'pinch') {
          const dist = Math.hypot(e.touches[0].clientX - e.touches[1].clientX, e.touches[0].clientY - e.touches[1].clientY);
          if (lastTouchDist > 0) distRef.current = Math.max(20, Math.min(200, distRef.current * (lastTouchDist / dist)));
          lastTouchDist = dist;
          const center = { x: (e.touches[0].clientX + e.touches[1].clientX) / 2, y: (e.touches[0].clientY + e.touches[1].clientY) / 2 };
          if (lastTouchCenter) {
            const ps = distRef.current * 0.003;
            lookAtRef.current.x -= (center.x - lastTouchCenter.x) * ps;
            lookAtRef.current.y += (center.y - lastTouchCenter.y) * ps;
          }
          lastTouchCenter = center; updateCamera();
        }
      };
      const onTouchEnd = (e) => {
        if (interactionMode === 'dragBox' && dragBoxId && e.changedTouches.length === 1) {
          const t = e.changedTouches[0];
          const moved = pointerDownPos ? Math.hypot(t.clientX - pointerDownPos.x, t.clientY - pointerDownPos.y) : 999;
          if (moved < 10) {
            const now = Date.now();
            if (now - lastTapTime < 400) cbRef.current.onRequestInlineEdit?.(dragBoxId);
            else cbRef.current.onSelectNode?.(dragBoxId);
          ...
          const t = e.changedTouches[0];
          if (Math.hypot(t.clientX - pointerDownPos.x, t.clientY - pointerDownPos.y) < 10) cbRef.current.onSelectNode?.(null);
        }
        interactionMode = null; dragBoxId = null; pointerDownPos = null; lastTouchDist = 0; lastTouchCenter = null;
      };
      const onWheel = (e) => { e.preventDefault(); distRef.current = Math.max(20, Math.min(200, distRef.current * (e.deltaY > 0 ? 1.1 : 0.9))); updateCamera(); };
      ...
      cleanup = () => {
        cancelled = true; setSceneReady(false);
  ...
  useEffect(() => {
    ...
    boxesRef.current = {};
    regionMeshesRef.current.forEach(m => { scene.remove(m); m.geometry.dispose(); m.material.dispose(); });
    ...
    // --- Regions (drawn behind boxes at z = -0.1 to -0.3) ---
    regions.forEach(reg => {
      const b = reg.bounds;
      const w = b.maxX - b.minX, h = b.maxY - b.minY;
      if (w < 1 || h < 1) return;
      const { hex, alpha } = getRegionColor(themeId, reg.color, reg.depth);
      const geom = new THREE.PlaneGeometry(w, h);
      const mat = new THREE.MeshBasicMaterial({
        color: hex, transparent: true, opacity: alpha,
        side: THREE.DoubleSide, depthWrite: false
      });
      const mesh = new THREE.Mesh(geom, mat);
      mesh.position.set(b.minX + w / 2, b.minY + h / 2, -0.1 - reg.depth * 0.1);
      mesh.renderOrder = -10 + reg.depth;
      scene.add(mesh);
      regionMeshesRef.current.push(mesh);

      // Region border (wireframe)
      const edges = new THREE.EdgesGeometry(geom);
      const lineMat = new THREE.LineBasicMaterial({ color: hex, transparent: true, opacity: alpha * 3 });
      const border = new THREE.LineSegments(edges, lineMat);
      border.position.copy(mesh.position);
      scene.add(border);
      regionMeshesRef.current.push(border);
    });
    ...
    links.forEach((link, idx) => {
      ...
      const curve = new THREE.QuadraticBezierCurve3(
        new THREE.Vector3(aM.position.x, aM.position.y, -0.7), mid,
  ...
  useEffect(() => {
    ...
    if (selectedId) {
      ...
      Object.entries(boxes).forEach(([id, m]) => { const c = conn.has(id); m.material.forEach(mt => { mt.transparent = !c; mt.opacity = c ? 1 : 0.25; }); });
    } else {
