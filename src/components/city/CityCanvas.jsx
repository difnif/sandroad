...
function clampView(v, cs, layout) { if (!layout.districts.length) return v; const b = getLayoutBounds(layout); const bw = b.maxX-b.minX+50, bh = b.maxY-b.minY+50; let { panX, panY, zoom } = v; const minZoom = Math.max(0.3, Math.min(cs.w/(bw*3), cs.h/(bh*3))); zoom = Math.max(minZoom, Math.min(4, zoom)); const m = 100; const cMinX = b.minX*zoom+panX, cMaxX = b.maxX*zoom+panX, cMinY = b.minY*zoom+panY, cMaxY = b.maxY*zoom+panY; if (cMaxX < -m) panX += (-m-cMaxX); if (cMinX > cs.w+m) panX -= (cMinX-cs.w-m); if (cMaxY < -m) panY += (-m-cMaxY); if (cMinY > cs.h+m) panY -= (cMinY-cs.h-m); return { panX, panY, zoom }; }
