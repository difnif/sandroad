...
export default function CityCanvas({
  ...
  const handleWheel = (e) => { e.preventDefault(); if (dragState) return; const f = e.deltaY > 0 ? 0.95 : 1.05; setViewState(prev => { const nz = Math.max(0.15, Math.min(5, prev.zoom*f)); const cx = canvasSize.w/2, cy = canvasSize.h/2; const r = nz/prev.zoom; return clampView({ zoom: nz, panX: cx-(cx-prev.panX)*r, panY: cy-(cy-prev.panY)*r }, canvasSize, layout); }); };
